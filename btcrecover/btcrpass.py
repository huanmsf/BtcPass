...
class WalletBIP39(object):
    ...
    def return_verified_password_or_false(self, passwords):
        ...
        for count, password in enumerate(passwords, 1):
            seed_bytes = pbkdf2_hmac("sha512", self._mnemonic, b"mnemonic" + password, 2048)
...
def error_exit(*messages):
    # 确保所有参数都是字符串类型
    prog_str = str(prog) if isinstance(prog, bytes) else str(prog)
    messages_str = [str(m) if isinstance(m, bytes) else str(m) for m in messages]
    result = _do_safe_print(prog_str + ": error:", *messages_str)
    # 确保结果中的所有元素都是字符串
    result_str = [str(item) if isinstance(item, bytes) else str(item) for item in result]
    sys.exit(" ".join(result_str))
...
def check_chars_range(s, error_msg, no_replacement_chars=False):
    assert isinstance(s, str), "check_chars_range: s is of " + str(str)
    if str == str:
        ...
        if sys.maxsize < 65536:  # 2**16
            for c in s:
                c = ord(c)
                if 0xD800 <= c <= 0xDBFF or 0xDC00 <= c <= 0xDFFF:
                    error_exit(error_msg, "has character with code point > max ("+str(sys.maxsize)+" / Unicode BMP)")
...
def expand_single_range(m):
    ...
    return str().join(map(chr, range(char_first, char_last+1)))
...
def count_valid_wildcards(str_with_wildcards, permit_contracting_wildcards = False):
    ...
    if str("%") in valid_wildcards_removed:
...
def parse_arguments(effective_argv, wallet = None, base_iterator = None,
    ...
    if not (args.passwordlist or tokenlist_file or args.performance or base_iterator or args.calc_memory):
        error_exit("argument --tokenlist or --passwordlist is required (or file "+str(TOKENS_AUTO_FILENAME)+" must be present)")
    ...
    if args.custom_wild:
        ...
        if (args.passwordlist or base_iterator) and not \
            ...
            wildcard_sets[str("c")] = custom_set_built  # (duplicates already removed by build_wildcard_set)
            wildcard_sets[str("C")] = duplicates_removed(custom_set_built.upper())
            ...
            if custom_set_caseswapped != custom_set_built:
                wildcard_nocase_sets[str("c")] = duplicates_removed(custom_set_built + custom_set_caseswapped)
                wildcard_nocase_sets[str("C")] = wildcard_nocase_sets[str("c")].swapcase()
            wildcard_keys += str("cC")  # keep track of available wildcard types (this is used in regex's)
    ...
    if args.exclude_passwordlist:
        ...
        # for exclude-passwordlist; instruct the password_dups to disable future duplicate tracking
...
def parse_mapfile(map_file, running_hash = None, feature_name = b"map", same_permitted = False):
    ...
    try:
        for line_num, line in enumerate(map_file, 1):
            ...
            if split_line in ([], [""]): continue  # ignore empty lines
            if len(split_line) == 1:
            ...
            check_chars_range(str().join(split_line), feature_name + b" file" + (b" '" + map_file.name + b"'" if hasattr(map_file, "name") else b""))
            for c in split_line[0]:  # (c is the character to be replaced)
                replacements = duplicates_removed(map_data.get(c, str()) + split_line[1])
...
#     [ AnchoredToken(begin=1, end=sys.maxsize) ],
#     [ AnchoredToken(begin=1, end=sys.maxsize) ],
        ...
        if token.startswith(b"^"):
            ...
            if match:
                ...
                if match.group(b"middle"):
                    ...
                    cached_str = str("^")  # begin building the cached __str__
                    if begin is None:
                        begin = 2
                    else:
                        begin = int(begin)
...
def parse_tokenlist(tokenlist_file, first_line_num = 1):
    ...
    for line_num, line in enumerate(tokenlist_file, first_line_num):
        ...
        # Ignore empty lines
        if new_list in ([None], [None, ""]): continue
...
def expand_wildcards_generator(password_with_wildcards, prior_prefix = None):
    if prior_prefix is None: prior_prefix = ""
    ...
    if m_bref:
        ...
        if first_pos < 0:  # if the prefix is shorter than the requested bpos
            wildcard_minlen = max(wildcard_minlen + first_pos, 0)
            wildcard_maxlen = max(wildcard_maxlen + first_pos, 0)
    ...
    # If it's an expanding wildcard
        ...
        for wildcard_len in range(wildcard_minlen, wildcard_maxlen+1):
    ...
    # Otherwise it's a contracting wildcard
        ...
        for remove_total in range(wildcard_minlen, min(wildcard_maxlen, max_from_left+max_from_right) + 1):
            ...
            for remove_right in range(max(0, remove_total-max_from_left), min(remove_total, max_from_right) + 1):
...
# the entire password_base (producing just one variation of the password_base in addition
...
def swap_typos_generator(password_base, min_typos = 0):
    ...
    for swap_count in range(max(1, min_typos), max_swaps + 1):
...
def simple_typos_generator(password_base, min_typos = 0):
    ...
    for typos_count in range(max(1, min_typos), max_typos + 1):
...
def insert_typos_generator(password_base, min_typos = 0):
    ...
    for inserts_count in range(max(1, min_typos), max_inserts + 1):
        ...
        for insert_indexes in combinations_function(range(password_base_len + 1), inserts_count):
